...
bool BG95ESP32::setBearerSetting(ATConstStr parameter, const char* value)
{
	sendFormatAT(TO_F(AT_COMMAND_SET_BEARER_SETTING_PARAMETER), parameter, value);
	return waitResponse() == 0;
}

bool BG95ESP32::getGprsPowerState(bool *state)
{
	uint8_t result;
...
bool BG95ESP32::enableGprs(const char *apn, const char* user, const char *password)
{
	char gprsToken[5];
...
BG95ESP32NetworkRegistrationState BG95ESP32::getNetworkRegistrationStatus()
{
	uint8_t stat;
	sendAT(TO_F(TOKEN_CGREG), TO_F(TOKEN_READ));
	
	if(waitResponse(TO_F(TOKEN_CGREG)) != 0 ||
		!parseReply(',', (uint8_t)BG95ESP32RegistrationStatusResponse::Stat, &stat) ||
		waitResponse() != 0)
		return BG95ESP32NetworkRegistrationState::Error;

	return (BG95ESP32NetworkRegistrationState)stat;
}
