...
protected:
  ...
  int8_t waitResponse(ATConstStr s1 = TO_F(TOKEN_OK), ATConstStr s2 = TO_F(TOKEN_ERROR), ATConstStr s3 = NULL, ATConstStr s4 = NULL) {
    return waitResponse(BG95AT_DEFAULT_TIMEOUT, s1, s2, s3, s4);
  };

  int8_t waitResponse(uint16_t timeout, ATConstStr s1 = TO_F(TOKEN_OK), ATConstStr s2 = TO_F(TOKEN_ERROR), ATConstStr s3 = NULL, ATConstStr s4 = NULL);
  ...
  /**
	 ...
	 */
  bool parseReply(char divider, uint8_t index, size_t *result) {
    return parse(replyBuffer, divider, index, result);
  }
#endif
  /**
...
#pragma region Stream
  ...
  size_t write(uint8_t x) {
    BG95ESP32_PRINT_CHAR(x);
    return _port->write(x);
  }
  int read() {
    return _port->read();
  }
