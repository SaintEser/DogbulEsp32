...
bool BG95ESP32::setupHttpRequest(const char *url) {
  ...
  return httpInit() && setHttpParameter(TO_F(AT_COMMAND_PARAMETER_HTTP_REDIR), 1) && setHttpParameter(TO_F(AT_COMMAND_PARAMETER_HTTP_CID), 1)
...
bool BG95ESP32::fireHttpRequest(const BG95ESP32HttpAction action, uint16_t *statusCode, size_t *dataSize) {
  ...
  return waitResponse(HTTP_TIMEOUT, TO_F(TOKEN_HTTP_ACTION)) == 0 && parseReply(',', (uint8_t)BG95ESP32HttpActionResponse::StatusCode, statusCode)
